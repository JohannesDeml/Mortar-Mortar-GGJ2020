// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CameraExtensions.cs" company="Supyrb">
...
namespace Supyrb
{
	...
	{
		/// <summary>
		/// Adds a layer to the culling mask of this camera
		/// </summary>
		/// <param name="camera">The camera the layer is added to</param>
		/// <param name="layerName">Name of the layer that is added to the culling mask</param>
		public static void AddCullingMaskLayer(this Camera camera, string layerName)
		{
		...
		public static Mesh GenerateFrustumMesh(this Camera cam)
		{
			...
			{
				Vector3 faceNormal =
					Vector3.Cross(vertices[i * 4 + 2] - vertices[i * 4 + 1], vertices[i * 4 + 0] - vertices[i * 4 + 1]).normalized;
